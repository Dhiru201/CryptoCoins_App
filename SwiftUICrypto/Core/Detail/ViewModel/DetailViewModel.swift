...
class DetailViewModel: ObservableObject {
    ...
    func addSubscribers() {
        ...
        coinDetailService.$coinDetails
            .sink {[weak self] (returedCoinDetails) in
                self?.coinDescription = returedCoinDetails?.readableDescription
                self?.websiteURL = returedCoinDetails?.links?.homepage?.first
                self?.redditURL = returedCoinDetails?.links?.subredditURL
            }
            .store(in: &cancellable)
    }
    ...
    private func createAdditionalArray(coinModel: CoinModel, coinDetailModel: CoinDetailModel?) -> [StatisticModel] {
        ...
        let highest = StatisticModel(title: "24h High", value: high)
        ...
        let marketCapPercentChange = coinModel.marketCapChangePercentage24H
        let marketCapChangeStat = StatisticModel(title: "24h Market Cap", value: marketCapChange, percentageChange: marketCapPercentChange)
        ...
        let additionalArray: [StatisticModel] = [
            highest, lowStat, priceChangeStat, marketCapChangeStat, blockStat, hashingStat
        ]
